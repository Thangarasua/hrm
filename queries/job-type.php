...
if (isset($_GET['flag']) && $_GET['flag'] === "fetch") {
    ...
    $query = "SELECT * FROM job_types WHERE job_type LIKE '%$data%'  ORDER BY job_type ASC";
...
if ($_SERVER['REQUEST_METHOD'] === "POST" && isset($_POST['flag'])) {
    ...
    if ($flag === "insert") {
        ...
        $query = "SELECT * FROM job_types WHERE job_type = '$value'";
        $result = mysqli_query($conn, $query);
        if (mysqli_num_rows($result) > 0) {
            echo json_encode(array('status' => 'failure', 'message' => 'Same value existing'));
            exit;
        } else {
            $query = "INSERT INTO `job_types` (`job_type`, `status`) VALUES ('$value', $status)";
            $result = mysqli_query($conn, $query);
            if ($result) {
